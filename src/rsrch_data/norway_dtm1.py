...
class NorwayDTM1:
    ...
    native 1m/px resolution. Since a download may only cover a random subset
    of tiles, `extent` bounds whatever is on disk -- gaps within it are
    ...
    def __init__(self, data_root: str | Path):
        ...
        tiles_dir = Path(data_root) / "tiles"
        paths = sorted(tiles_dir.glob("*.tif"))
        if not paths:
            msg = f"No tiles found in {tiles_dir}"
    ...
    def extent(self) -> tuple[float, float, float, float]:
        """Bounding box (left, bottom, right, top) in EPSG:25833 meters."""
    ...
    def get_tile(
        ...
        """Get the heightmap over `rect` (left, bottom, right, top), EPSG:25833 meters.
